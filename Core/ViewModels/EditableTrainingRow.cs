...
public partial class EditableTrainingRow : ObservableObject
{
    ...
    /// <summary>
    /// Parent-supplied callbacks. Bound buttons live inside a nested DataTemplate
    /// whose BindingContext is THIS row, so a direct {Binding AttendCommand}
    /// always resolves — unlike RelativeSource/x:Reference walks up out of the
    /// template, which fail silently across DataTemplate namescopes.
    /// </summary>
    public Func<EditableTrainingRow, Task>? AttendAction { get; set; }
    public Func<EditableTrainingRow, Task>? SaveAction { get; set; }
    public Func<EditableTrainingRow, Task>? DeleteAction { get; set; }

    [RelayCommand]
    private Task Attend() => AttendAction?.Invoke(this) ?? Task.CompletedTask;

    [RelayCommand]
    private Task Save() => SaveAction?.Invoke(this) ?? Task.CompletedTask;
